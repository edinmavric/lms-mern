...
export function ProfessorLessonMaterialEdit() {
  ...
  useEffect(() => {
    if (material) {
      const materialProfessorId =
        typeof material.professor === 'string'
          ? material.professor
          : material.professor._id;

      if (materialProfessorId !== user?._id) {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      navigate(`/app/professor/lesson-materials/${id}`);
    },
    onError: error => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => navigate(`/app/professor/lesson-materials/${id}`)}
        >
